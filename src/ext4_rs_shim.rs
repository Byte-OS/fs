...
impl BlockDevice for Ext4Disk {
    fn read_offset(&self, offset: usize) -> Vec<u8> {
        ...
        for i in 0..(BLOCK_SIZE / 512) {
            ...
            buf[buf_start..buf_end].copy_from_slice(&data[offset_in_block..(offset_in_block + bytes_to_copy)]);
    ...
    fn write_offset(&self, offset: usize, buf: &[u8]) {
        ...
        for i in 0..((bytes_to_write + 511) / 512) { // round up to cover partial blocks
            let current_block_id = start_block_id + i;
            ...
            data[offset_in_block..offset_in_block + bytes_to_copy].copy_from_slice(&buf[buf_start..buf_end]);
...
impl INodeInterface for Ext4FileWrapper {
    fn open(&self, path: &str, _flags: vfscore::OpenFlags) -> VfsResult<Arc<dyn INodeInterface>> {
        ...
        let r = self.ext4.ext4_open(&mut ext4_file, path, parse_flags, create);
